...
class EmptyController extends Common
{
    ...
    public function index()
    {
        ...
        isSinglePage(DBNAME)) {
            ...
            if (DBNAME == 'picture' || DBNAME == 'case') {
                //TODO moduleid 是模型的编号
                $setup = db('field')->where(array('moduleid' => 11, 'field' => 'group'))->value('setup');
    ...
    public function info()
    {
        ...
        $alsoLikes = $this->dao
            ->alias('a')
            ->join(config('database.prefix') . 'category c', 'a.catid = c.id', 'left')
//            ->where("a.catid=" . input('catId') . " AND a.id != " . input('id'))
            ->where("a.id != " . input('id'))
            ->field('a.*,c.catdir')
            ->order('listorder asc,createtime desc')
            ->limit(4)
            ->select();
//        $alsoLikes  = $alsoLikes ->toArray();
//        foreach ($alsoLikes as $vo) {
//             $pics = preg_grep("/;/", $vo['pictures']);
//             if(sizeof($pics)>=2){
//                 $vo['pic_al']
//             }
//        }
//        $this->assign('alsoLikes', $alsoLikes ['data']);
        $this->assign('alsoLikes', $alsoLikes);
